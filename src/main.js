...
const GAME_VERSION = "STAGE 4 · v4.0.0";
...
info.innerHTML = `<strong>${GAME_VERSION}</strong><br>1 AR-01 · 2 REVOLVER · 좌클릭 발사 · R 재장전 · T 적 리셋`;

document.querySelector(".panel h1").textContent = "HTML FPS — Stage 4";
document.querySelector(".panel p").textContent = "적 HP / 몸통·머리 히트박스 / 데미지 / 사망 테스트";
document.querySelector(".controls").innerHTML = `
  <span>WASD — 이동</span><span>Mouse — 시점</span>
  <span>Space — 점프</span><span>좌클릭 — 발사</span>
  <span>1 — AR-01</span><span>2 — REVOLVER</span>
  <span>R — 재장전</span><span>T — 적 리셋</span>`;
...
runtimeStyle.textContent = `
  ...
  #hitFeedback { position:absolute; left:50%; top:calc(50% + 35px); transform:translate(-50%,-50%) scale(.8); font-size:13px; font-weight:950; letter-spacing:.12em; opacity:0; transition:opacity .07s, transform .07s; text-shadow:0 2px 7px rgba(0,0,0,.65); }
  #enemyHud { position:absolute; left:18px; top:82px; padding:9px 12px; border-radius:9px; background:rgba(0,0,0,.45); font-size:12px; font-weight:850; letter-spacing:.08em; font-variant-numeric:tabular-nums; }
  #clearNotice { position:absolute; left:50%; top:28%; transform:translate(-50%,-50%) scale(.96); padding:12px 18px; border-radius:11px; background:rgba(0,0,0,.58); font-size:18px; font-weight:950; letter-spacing:.12em; opacity:0; transition:.18s ease; }
...
const enemySpawnPoints = [
  { x:0, z:9.0, rotationY:Math.PI },
  { x:-6.8, z:5.5, rotationY:Math.PI*0.7 },
  { x:7.4, z:7.5, rotationY:-Math.PI*0.75 },
  { x:-7.0, z:-7.8, rotationY:Math.PI*0.2 },
  { x:6.8, z:-8.2, rotationY:-Math.PI*0.2 },
  { x:0, z:-9.3, rotationY:0 },
];
...
function removeFromRaycast(mesh){ const index=raycastWorld.indexOf(mesh); if(index!==-1)raycastWorld.splice(index,1); }
...
function showHit(isHeadshot){
  hitFeedback.textContent=isHeadshot?"HEADSHOT":"HIT"; hitFeedback.style.color=isHeadshot?"#ffd27a":"#ffffff"; hitFeedback.style.opacity="1"; hitFeedback.style.transform="translate(-50%,-50%) scale(1.25)";
  setTimeout(()=>{hitFeedback.style.opacity="0";hitFeedback.style.transform="translate(-50%,-50%) scale(.8)";},isHeadshot?120:82);
}
function killEnemy(enemy){ if(!enemy.alive)return; enemy.alive=false; enemy.dying=true; enemy.deathStartedAt=performance.now(); enemy.hp=0; updateEnemyHealthBar(enemy); enemy.healthBar.group.visible=false; enemy.hitboxes.forEach(removeFromRaycast); updateEnemyHUD(); }
function damageEnemy(enemy,zone,weapon,hitPoint){
  if(!enemy||!enemy.alive)return; const isHeadshot=zone==="head"; const damage=isHeadshot?weapon.headDamage:weapon.damage; enemy.hp=Math.max(0,enemy.hp-damage); enemy.hitKick=1;
  const shotDirection=new THREE.Vector3(); camera.getWorldDirection(shotDirection); enemy.hitDirection.copy(shotDirection).multiplyScalar(0.08);
  flashEnemy(enemy,isHeadshot); showHit(isHeadshot); spawnDamageNumber(hitPoint,damage,isHeadshot); updateEnemyHealthBar(enemy); if(enemy.hp<=0)killEnemy(enemy);
}
function clearEnemies(){
  for(const enemy of enemies){ enemy.hitboxes.forEach(removeFromRaycast); scene.remove(enemy.root); enemy.root.traverse(object=>{if(object.isMesh&&object.geometry)object.geometry.dispose();}); enemy.materials.forEach(material=>material.dispose()); }
  enemies.length=0;
}
function spawnTestEnemies(){ clearEnemies(); enemySerial=0; enemySpawnPoints.forEach(point=>createEnemy(point.x,point.z,point.rotationY)); clearNotice.classList.remove("show"); updateEnemyHUD(); }
function updateEnemyHUD(){ const alive=enemies.filter(enemy=>enemy.alive).length; enemyHud.textContent=`ENEMIES ${alive} / ${enemies.length}`; clearNotice.classList.toggle("show",enemies.length>0&&alive===0); }
function updateEnemies(delta,now){
  for(const enemy of enemies){
    if(enemy.alive){
      enemy.healthBar.group.quaternion.copy(camera.quaternion); const hitDecay=1-Math.exp(-18*delta); enemy.hitKick=THREE.MathUtils.lerp(enemy.hitKick,0,hitDecay); const kick=enemy.hitKick;
      enemy.root.position.copy(enemy.basePosition).addScaledVector(enemy.hitDirection,kick); enemy.root.rotation.y=enemy.baseRotationY; enemy.root.rotation.z=Math.sin(now*0.035+enemy.id)*0.012*kick; continue;
    }
