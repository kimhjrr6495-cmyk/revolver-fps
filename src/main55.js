...
const GAME_VERSION = "STAGE 5.5 · MIDPOINT POLISH";
...
sun.position.set(9, 17, 7); sun.castShadow = true; sun.shadow.mapSize.set(1024, 1024);
...
document.querySelector(".panel h1").textContent = "HTML FPS — Stage 5.5";
document.querySelector(".panel p").textContent = "Midpoint Polish · Clean Combat Feedback";
...
info.innerHTML = `<strong>${GAME_VERSION}</strong><br><span style="opacity:.66">P 설정 · T 테스트 적 리셋</span>`;

const style = document.createElement("style");
style.textContent = `
  :root { --hud-bg:rgba(8,12,17,.56); --hud-line:rgba(255,255,255,.12); --accent:#e9f2f8; --danger:#ff4f5f; --head:#ffd067; }
  ...
  #hitBracket{position:absolute;left:50%;top:50%;width:50px;height:50px;transform:translate(-50%,-50%) scale(.72);opacity:0;pointer-events:none;transition:opacity .045s,transform .07s}
  .hb{position:absolute;width:12px;height:12px;border-color:var(--danger);border-style:solid;filter:drop-shadow(0 0 4px rgba(255,79,95,.35))}.hb.a{left:2px;top:2px;border-width:2px 0 0 2px}.hb.b{right:2px;top:2px;border-width:2px 2px 0 0}.hb.c{left:2px;bottom:2px;border-width:0 0 2px 2px}.hb.d{right:2px;bottom:2px;border-width:0 2px 2px 0}
  #hitBracket.head .hb{border-color:var(--head);filter:drop-shadow(0 0 5px rgba(255,208,103,.5))}
  #hitBracket.show{opacity:1;transform:translate(-50%,-50%) scale(1)}
  #combatText{position:absolute;left:50%;top:calc(50% + 48px);transform:translate(-50%,-50%);font-size:10px;font-weight:900;letter-spacing:.2em;opacity:0;transition:opacity .06s,transform .08s;text-shadow:0 2px 8px rgba(0,0,0,.65)}
  #combatText.show{opacity:.92;transform:translate(-50%,-52%)}
  ...
  #weaponName{font-size:10px;font-weight:900;letter-spacing:.16em;opacity:.65;text-align:right}
  #ammoRow{display:flex;align-items:flex-end;justify-content:flex-end;gap:6px;margin-top:3px;line-height:1}#ammoCurrent{font-size:39px;font-weight:950}#ammoMax{font-size:15px;font-weight:850;opacity:.45;margin-bottom:4px}
  ...
  #killPulse{position:absolute;left:50%;top:50%;width:72px;height:72px;border:1px solid rgba(255,79,95,.72);border-radius:50%;transform:translate(-50%,-50%) scale(.55);opacity:0;pointer-events:none}
  #killPulse.show{animation:killPulse .22s ease-out}
...
const settingsPanel=document.createElement("div");settingsPanel.id="settingsPanel";settingsPanel.innerHTML=`<div id="settingsCard"><h2>설정</h2><p>전투 화면은 최대한 비우고, 필요한 피드백만 남긴 중간점검 버전.</p><div class="setting"><span>마우스 감도</span><strong id="sensValue"></strong><input id="sensSlider" type="range" min="0.2" max="3" step="0.05"></div><div class="setting"><span>데미지 숫자</span><button id="damageToggle" class="toggle"></button></div><div class="setting"><span>명중 확인음</span><button id="soundToggle" class="toggle"></button></div><div class="settings-actions"><button id="resetSettings">기본값</button><button id="closeSettings">완료</button></div></div>`;document.body.appendChild(settingsPanel);
...
function createEnemy(x,z,rot){const e={id:++enemySerial,maxHp:100,hp:100,alive:true,dying:false,deathAt:0,hitKick:0,hitboxes:[],materials:[],root:new THREE.Group(),base:new THREE.Vector3(x,0,z),rot};e.root.position.copy(e.base);e.root.rotation.y=rot;scene.add(e.root);const body=enemyMat(0x41566b),head=enemyMat(0xc2a383),limb=enemyMat(0x324455),accent=enemyMat(0xa23d4b);e.materials.push(body,head,limb,accent);const torso=new THREE.Mesh(new THREE.BoxGeometry(.7,.9,.4),body);torso.position.y=1.13;e.root.add(torso);tag(torso,e,"body");const h=new THREE.Mesh(new THREE.BoxGeometry(.44,.44,.4),head);h.position.y=1.84;e.root.add(h);tag(h,e,"head");for(const [px,py,sx,sy] of [[-.2,.36,.23,.7],[.2,.36,.23,.7],[-.47,1.13,.18,.75],[.47,1.13,.18,.75]]){const p=new THREE.Mesh(new THREE.BoxGeometry(sx,sy,.25),limb);p.position.set(px,py,0);e.root.add(p);tag(p,e,"body");}const mark=new THREE.Mesh(new THREE.BoxGeometry(.16,.06,.02),accent);mark.position.set(0,1.86,-.21);e.root.add(mark);tag(mark,e,"head");const hp=makeHpBar();e.hpBar=hp;e.root.add(hp.group);e.root.traverse(o=>{if(o.isMesh)o.castShadow=true;});enemies.push(e);return e;}
...
function addFeed(text,head=false){const d=document.createElement("div");d.className="feed";d.style.color=head?"#ffd067":"#eef4f7";d.textContent=text;killFeed.prepend(d);while(killFeed.children.length>3)killFeed.lastElementChild.remove();setTimeout(()=>d.remove(),2200);}
function damageNumber(point,n,head){if(!settings.damageNumbers)return;const p=point.clone().project(camera);if(p.z<-1||p.z>1)return;const d=document.createElement("div");d.className="damage-number";d.style.left=`${(p.x*.5+.5)*innerWidth}px`;d.style.top=`${(-p.y*.5+.5)*innerHeight}px`;d.style.color=head?"#ffd067":"#fff";d.textContent=n;hud.appendChild(d);setTimeout(()=>d.remove(),500);}

let hitTimer=null,textTimer=null;
function showHitFeedback({head=false,kill=false}){hitBracket.classList.toggle("head",head);hitBracket.classList.add("show");combatText.textContent=kill?(head?"HEADSHOT · ELIMINATED":"ELIMINATED"):(head?"HEADSHOT":"");combatText.style.color=head?"#ffd067":"#ff6572";if(combatText.textContent)combatText.classList.add("show");clearTimeout(hitTimer);clearTimeout(textTimer);hitTimer=setTimeout(()=>hitBracket.classList.remove("show"),head?135:80);textTimer=setTimeout(()=>combatText.classList.remove("show"),kill?420:head?160:70);if(kill){killPulse.classList.remove("show");void killPulse.offsetWidth;killPulse.classList.add("show");}}
...
function cleanTick(head=false,kill=false){if(!settings.hitSound)return;const a=audio(),now=a.currentTime;const tone=(freq,start,dur,vol,type="sine")=>{const o=a.createOscillator(),g=a.createGain();o.type=type;o.frequency.setValueAtTime(freq,start);g.gain.setValueAtTime(vol,start);g.gain.exponentialRampToValueAtTime(.001,start+dur);o.connect(g).connect(a.destination);o.start(start);o.stop(start+dur+.01);};tone(head?1250:940,now,.045,head?.045:.032,"triangle");if(head)tone(kill?1760:1540,now+.014,.055,kill?.05:.036,"sine");if(kill)tone(620,now+.02,.07,.028,"triangle");}
function flashEnemy(e,head){const c=head?0xffb85e:0xff6572;e.materials.forEach(m=>m.emissive.setHex(c));setTimeout(()=>{if(e.alive)e.materials.forEach(m=>m.emissive.setHex(0));},65);}
...
function damageEnemy(e,zone,w,point){if(!e?.alive)return;const head=zone==="head",damage=head?w.headDamage:w.damage,kill=e.hp-damage<=0;e.hp=Math.max(0,e.hp-damage);e.hitKick=1;updateHp(e);flashEnemy(e,head);damageNumber(point,damage,head);showHitFeedback({head,kill});cleanTick(head,kill);if(head)shakePower=Math.min(1,shakePower+(kill?.26:.14));if(kill)killEnemy(e,head,w);}
function updateEnemies(delta,now){for(const e of enemies){if(e.alive){e.hpBar.group.quaternion.copy(camera.quaternion);e.hitKick=THREE.MathUtils.lerp(e.hitKick,0,1-Math.exp(-17*delta));e.root.position.copy(e.base);e.root.position.z+=e.hitKick*.06;e.root.rotation.y=e.rot;continue;}if(e.dying){const t=THREE.MathUtils.clamp((now-e.deathAt)/480,0,1),q=1-Math.pow(1-t,3);e.root.rotation.z=-q*1.25;e.root.position.y=-q*.18;e.root.scale.setScalar(1-q*.07);if(t>=1)e.dying=false;}}}
...
function part(parent,g,m,p,r=[0,0,0],s=[1,1,1]){const x=new THREE.Mesh(g,m);x.position.set(...p);x.rotation.set(...r);x.scale.set(...s);parent.add(x);return x;}
...
const keys=new Set();window.addEventListener("keydown",e=>{keys.add(e.code);if(e.code==="Space"&&player.grounded&&document.pointerLockElement===renderer.domElement){player.velocityY=player.jumpSpeed;player.grounded=false;}if(e.code==="KeyR")startReload();if(e.code==="Digit1")beginSwitch("assaultRifle");if(e.code==="Digit2")beginSwitch("revolver");if(e.code==="KeyT")spawnEnemies();if(e.code==="KeyP")openSettings();});window.addEventListener("keyup",e=>keys.delete(e.code));window.addEventListener("blur",()=>{keys.clear();mouseHeld=false;});window.addEventListener("mousedown",e=>{if(e.button!==0)return;mouseHeld=true;if(!currentWeapon().automatic)fire();});window.addEventListener("mouseup",e=>{if(e.button===0)mouseHeld=false;});
startButton.onclick=()=>{audio();renderer.domElement.requestPointerLock();};renderer.domElement.onclick=()=>{audio();if(document.pointerLockElement!==renderer.domElement&&!settingsPanel.classList.contains("show"))renderer.domElement.requestPointerLock();};document.addEventListener("pointerlockchange",()=>{const locked=document.pointerLockElement===renderer.domElement;startScreen.classList.toggle("hidden",locked);if(!locked){keys.clear();mouseHeld=false;}});document.addEventListener("mousemove",e=>{if(document.pointerLockElement!==renderer.domElement)return;const s=.0021*settings.sensitivity;yaw-=e.movementX*s;pitch-=e.movementY*s;pitch=THREE.MathUtils.clamp(pitch,-Math.PI/2+.01,Math.PI/2-.01);});

function vOverlap(b){return player.position.y+player.height>b.minY&&player.position.y<b.maxY;}function collides(x,z){const r=player.radius;return colliders.some(b=>vOverlap(b)&&x+r>b.minX&&x-r<b.maxX&&z+r>b.minZ&&z-r<b.maxZ);}function move(dx,dz){const nx=player.position.x+dx;if(!collides(nx,player.position.z))player.position.x=nx;const nz=player.position.z+dz;if(!collides(player.position.x,nz))player.position.z=nz;}
const forward=new THREE.Vector3(),right=new THREE.Vector3(),wish=new THREE.Vector3();function updateMove(d){let x=0,z=0;if(keys.has("KeyW"))z++;if(keys.has("KeyS"))z--;if(keys.has("KeyD"))x++;if(keys.has("KeyA"))x--;forward.set(-Math.sin(yaw),0,-Math.cos(yaw));right.set(Math.cos(yaw),0,-Math.sin(yaw));wish.set(0,0,0).addScaledVector(forward,z).addScaledVector(right,x);if(wish.lengthSq()){wish.normalize();move(wish.x*player.moveSpeed*d,wish.z*player.moveSpeed*d);}player.velocityY+=player.gravity*d;player.position.y+=player.velocityY*d;if(player.position.y<=0){player.position.y=0;player.velocityY=0;player.grounded=true;}else player.grounded=false;}
...
let fpsTime=0,fpsFrames=0;const clock=new THREE.Clock();function loop(){requestAnimationFrame(loop);const d=Math.min(clock.getDelta(),.05),now=performance.now();if(document.pointerLockElement===renderer.domElement){updateMove(d);if(mouseHeld&&!switching.active&&currentWeapon().automatic)fire();}updateReload(now);const drop=updateSwitch(now);updateWeapon(d,now,drop);updateCamera(d);updateEnemies(d,now);syncCamera();fpsTime+=d;fpsFrames++;if(fpsTime>=.5){fpsElement.textContent=`FPS: ${Math.round(fpsFrames/fpsTime)}`;fpsTime=0;fpsFrames=0;}stateElement.textContent=player.grounded?"GROUND":"AIR";renderer.render(scene,camera);}spawnEnemies();updateWeaponHud();syncCamera();loop();
